...
public class UserService extends userGrpc.userImplBase {
    ...
    String databaseURL = "jdbc:mysql://localhost:3306/grpc_server_client";
    String user = "root";
    String pass = "";
    ...
    public void userRegistration(User.RegistrationRequest request, StreamObserver<User.RegistrationResponse> responseObserver) throws SQLException {
        ...
        String city = request.getCountry();
        ...
        while (resultSet.next()) {
            if(resultSet.getInt(1) == 1) {
                ...
                PreparedStatement statement = connection.prepareStatement
                        ("INSERT INTO tbl_info VALUES('"+userID+"', '"+name+"', '"+ city +"')");
                statement.executeUpdate();
                response.setResponseMessage(name +
                                " with User ID " + userID + " from " + city + " has completed registration").
                        setResponseCode(300);
            }
            break;
        }
    ...
    private ResultSet checkPersonalInfo(int userID) throws SQLException {
        Connection connection = getConnection(databaseURL, user, pass);
        PreparedStatement statement = connection.prepareStatement("SELECT EXISTS(SELECT * FROM tbl_info WHERE userID = ?)");
